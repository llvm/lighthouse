...
def execute(
    ...
    payload_module = lower_payload(
        workload, schedule_parameters=schedule_parameters
    )
    # get execution engine
    engine = get_engine(
        payload_module, requirements=workload.requirements()
    )
...
def emit_benchmark_function(
    ...
    for op in payload_module.operation.regions[0].blocks[0]:
        if (isinstance(op, func.FuncOp) and
                str(op.name).strip('"') == workload.payload_function_name):
            payload_func = op
            break
    ...
    with workload.context, workload.location:
        ...
        with ir.InsertionPoint(f.add_entry_block()):
            ...
            with ir.InsertionPoint(for_op.body):
                func.CallOp(
                    payload_func, list(f.arguments[:len(payload_arguments)])
                )
                scf.YieldOp(())
...
def benchmark(
    ...
    engine = get_engine(
        payload_module, requirements=requirements
    )
